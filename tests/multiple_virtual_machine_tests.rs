...
fn test_basic_connection_between_vms() {
    ...
    let handel1 = thread::spawn(move || {
        ...
        vm1.connect_to_connection(0, &mut connection_to_vm1);
    ...
    let handel2 = thread::spawn(move || {
        ...
        vm2.connect_to_connection(3, &mut connection_to_vm2);
...
fn test_basic_communication_between_vms() {
    ...
    let handel1 = thread::spawn(move || {
        let mut vm1 = VirtualMachine::new();
        vm1.connect_to_connection(0, &mut communication_connection_vm1);
        vm1.connect_to_connection(1, &mut data_connection_vm1);
    ...
    let handel2 = thread::spawn(move || {
        ...
        let mut vm2 = VirtualMachine::new();
        vm2.connect_to_connection(0, &mut communication_connection_vm2);
        vm2.connect_to_connection(1, &mut data_connection_vm2);
...
fn test_basic_communication_between_vms_with_simpler_api() {
...

    {
        vm1.lock()
            .unwrap()
            .connect_to_connection(0, &mut communication_coon_to_vm1);
    }

    {
        vm1.lock()
            .unwrap()
            .connect_to_connection(1, &mut data_conn_vm1);
    }

    {
        vm2.lock()
            .unwrap()
            .connect_to_connection(0, &mut communication_coon_to_vm2);
    }
    {
        vm2.lock()
            .unwrap()
            .connect_to_connection(1, &mut data_conn_vm2);
    }
...
fn test_connecting_port_to_port() {
    ...
    consume:            #   block of code responsible for input consumption
        MOV p1 acc      #   copying value form p1 to acc
        DIV 100         #   dividing received value by 8
        HLT             #
    HLT                 #
    "#;
    ...
    VirtualMachine::connect_vm_ports(vm1.clone(), vm2.clone(), 0, 0);
    VirtualMachine::connect_vm_ports(vm1.clone(), vm2.clone(), 1, 1);
